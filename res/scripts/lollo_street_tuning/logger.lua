...
return {
    ...
    warn = function(label, ...)
        ...
        print('lollo_street_tuning WARNING: ' .. label, ...)
    end,
    err = function(label, ...)
        if not(_isErrorLogActive) then return end
        print('lollo_street_tuning ERROR: ' .. label, ...)
    end,
    ...
    xpHandler = function(error)
        if not(_isExtendedLogActive) then return end
        print('lollo_street_tuning INFO:') debugPrint(error)
    end,
    xpWarningHandler = function(error)
        if not(_isWarningLogActive) then return end
        print('lollo_street_tuning WARNING:') debugPrint(error)
    end,
    xpErrorHandler = function(error)
        if not(_isErrorLogActive) then return end
        print('lollo_street_tuning ERROR:') debugPrint(error)
    end,
}
